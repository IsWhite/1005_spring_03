...
<table>
    ...
    <%--不用ajax请求--%>
    <%--每次生成的对象stu--%>
    <s:iterator value="#session.pageBean.beanList"
                var="stu">
        <tr>
            <td>${stu.sid}</td>
            <td>${stu.sname}</td>
            <td>${stu.clazz.cname}</td>
            <td>${stu.clazz.cinfor}</td>
        </tr>
    </s:iterator>
    </tbody>
</table>
<div>
    <span>第${session.pageBean.pageNum}/
        ${session.pageBean.totalPage}页</span>
    <a href="findStudent.action?pageNum=1&pageSize=${session.pageBean.pageSize}">[首页]</a>
    <a href="findStudent.action?pageNum=${session.pageBean.pageNum-1}&pageSize=${session.pageBean.pageSize}">[上一页]</a>
    <a href="findStudent.action?pageNum=${session.pageBean.pageNum+1}&pageSize=${session.pageBean.pageSize}">[下一页]</a>
    <a href="findStudent.action?pageNum=${session.pageBean.totalPage}&pageSize=${session.pageBean.pageSize}">[尾页]</a>
</div>
<%--引入jquery--%>
